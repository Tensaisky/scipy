...
svr = joblib.load('C:/Users/WORK01/Desktop/scipy/com/usst/svr2.pkl')
...
if (date_start == daten):
    label = df[(df.DATA_TIME > date_start) & (df.DATA_TIME < datet)].iloc[:, 2:3]
    precession = 0
else:
    label = df[(df.DATA_TIME > date_start) & (df.DATA_TIME < date_end)].iloc[:, 2:3]
